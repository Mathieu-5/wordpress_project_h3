	...
	switch ( $action ) {
		case 'delete' :
			...
			elseif ( $pid != '' ) {
				...
				$petition = __( 'All Signatures', 'speakout' );
			}
			...
			$message_update = __( 'Signature deleted.', 'speakout' );
		...
		case 'petition' :
			...
			if ( count( $signatures ) > 0 ) {
				...
				$table_label = __( 'Petition', 'speakout' ) . ' ' . $pid . ' <span class="count">(0)</span>';
			}
			$base_url      = site_url( 'wp-admin/admin.php?page=dk_speakout_signatures&action=petition&pid=' . $pid );
			$message_update = '';
		break;
		...
		default :
			...
			// set up display strings
			$table_label = __( 'All Signatures', 'speakout' ) . ' <span class="count">(' . $count . ')</span>';
