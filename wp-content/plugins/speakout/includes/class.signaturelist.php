...
if($options['signaturelist_display']=='list'){
    ...
    foreach ( $signatures as $signature ) {
		...
		$signatures_list .= stripslashes($signature->honorific . " " . $signature->first_name . ' ' . $display_lastname ); 
	}
...
// or display signatures as a table
			...
			foreach ( $signatures as $signature ) {
				...
				$signatures_list .= "\t" .'<td class="dk-speakout-signaturelist-name" >' . stripslashes( $signature->honorific . " " . $signature->first_name . ' ' . $display_lastname ) . '</td>'.PHP_EOL ;
