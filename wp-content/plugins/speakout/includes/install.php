...
function dk_speakout_install() {
	...
	$sql_create_tables = "
		CREATE TABLE `$db_petitions` (
			...
			`target_email_CC` LONGTEXT CHARACTER SET utf8 NOT NULL,
			...
			`petition_footer` LONGTEXT CHARACTER SET utf8 NOT NULL,
			...
			`twitter_message` VARCHAR(260) CHARACTER SET utf8 NOT NULL,
			...
			`custom_field_location` INT NOT NULL DEFAULT '1',
		...
		CREATE TABLE `$db_signatures` (
			...
			`IP_address` VARCHAR(48) CHARACTER SET utf8 NULL,
			UNIQUE KEY  (`id`)
		);";
...
$sql_update = "
			ALTER TABLE $db_petitions ADD `custom_field_location` INT NOT NULL DEFAULT '1' AFTER `custom_field_label`;
		;";
...
$sql_update = "
			ALTER TABLE $db_petitions ADD `petition_footer` LONGTEXT NOT NULL AFTER `petition_message`;
		;";
		//echo $sql_update; exit;
		$wpdb->query( $sql_update );
...
$sql_update = "
			ALTER TABLE $db_petitions ADD `target_email_CC` LONGTEXT NOT NULL AFTER `target_email`;
		;";
		//echo $sql_update; exit;
		$wpdb->query( $sql_update );
...
if ( version_compare( $installed_version, '1.9.0', '<' ) == 1 ) {
	...
	if ( $installed_version != $dk_speakout_version ) { 
		...
		if ( ! array_key_exists( 'display_recaptcha', $options ) ) {
			$options['display_recaptcha'] = "enabled";
		}
