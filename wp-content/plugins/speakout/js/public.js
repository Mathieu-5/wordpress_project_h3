...
*/
	$( '.dk-speakout-submit' ).click( function( e ) {
		...
		var id             = $( this ).attr( 'name' ),
			...
			optin          = '',
			bcc            = '',
			privacypolicy  = $( '#dk-speakout-privacypolicy').is(':checked'),
		...
		if ( $('#dk-speakout-privacypolicy').length && privacypolicy === false ){
			$( '#dk-speakout-privacypolicy' ).addClass( 'dk-speakout-error' ); 
			errors ++;
		}
		...
		if ( errors === 0 && $( this ).attr( 'rel' ) !== 'disabled' ) {
			...
			$.post( dk_speakout_js.ajaxurl, data,
				function( response ) {
					...
					// if you would like the petion form to vanish after it is signed, comment out the line above
					// and uncomment the line below.  You will have to do this any time this page is upgraded (rarely).
