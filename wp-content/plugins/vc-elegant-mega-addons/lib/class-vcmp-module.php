...
if ( !class_exists ('VcmpModule')) {
	abstract class VcmpModule extends VcmpBase {
		...
		public function settings_section_callback($args){
			do_action('vcmp_module_settings_section_callback', $args);
		}
		...
		protected function render_setting_field($args){
			...
			if(!$field['type']){
				echo __('Error: Setting field type is empty!', VCMP_SLUG);
				return;
			}

			if(!$field['name']){
				echo __('Error: Setting field name is empty!', VCMP_SLUG);
				return;
			}
			...
			switch ($field['type']) {
				...
				default:
						do_action('vcmp_module_render_setting_field', $field);
					break;
			}
