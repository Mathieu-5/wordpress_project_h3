...
/*
 * VCMP Module: 3D Profile Box
 * Description: 3D Profile Box shortcode for Visual Composer
 * Author Name: ThemeofWP.com
 * Author URL: http://themeofwp.com/
 * Version: 1.0.0
 */

class VcmpProfileBoxes extends VcmpModule{

	const slug = 'vcmp_profilebox';
	const base = 'vcmp_profilebox';
    ...
    public function vc_before_init(){
        vc_map( array(
            "name" => __("3D Profile Box", "VCMP_SLUG"),
            ...
            //'admin_enqueue_js' => array(plugins_url('assets/vc_vcmpprofilebox.js', __FILE__)), // This will load js file in the VC backend editor
            //'admin_enqueue_css' => array(plugins_url('assets/vc_vcmpprofilebox_admin.css', __FILE__)), // This will load css file in the VC backend editor
            "params" => array(
			
				array( 
						"type" => "attach_image",
						"heading" => __( "Box Background Image", "VCMP_SLUG" ),
						"param_name" => "vcmpprofilebox_bgimg",
				...
				array( 
						...
						"param_name" => "vcmpprofilebox_bgcolor",
				...
				array( 
						...
						"param_name" => "vcmpprofilebox_title",
    ...
    */
    public function build_shortcode( $atts, $content = null ) {
	  ...
	  $output = $el_class = $vcmpprofilebox_title = $use_theme_fonts  =  $google_fonts   =  $use_custom_for_content = $google_fonts_data = $font_container_data  =  $font_container = $vcmpprofilebox_bgimg = $vcmpprofilebox_bgcolor = $vcmpprofilebox_url = $el_class = $css = '';

		extract(shortcode_atts( array(
			'el_class' => '',
			'vcmpprofilebox_title' => '',
			'use_custom_for_content' => '',
			'vcmpprofilebox_bgimg' => '',
			'vcmpprofilebox_bgcolor' => '',
			'vcmpprofilebox_url' => '',
			'font_container' => '',
			'use_theme_fonts' => '',
			'google_fonts' => '',
		...
		wp_enqueue_style( 'vc_profilebox', VCMP_URL . 'modules/vcmp-3d-profile-box/assets/css/vc_3dprofilebox.css');
		wp_enqueue_script( 'vc_profilebox', VCMP_URL.'modules/vcmp-3d-profile-box/assets/js/vc_3dprofilebox.js', array('jquery'), '1.0', TRUE);
		
		$vc_vcmpprofilebox_bigthumbnails= array();
		$images = explode(',', $vcmpprofilebox_bgimg);
        ...
        $vc_vcmpprofilebox_bigthumbnails[$key] = $bigimage_array[0];
		$alt = get_post_meta(intval($image), '_wp_attachment_image_alt', true);
		
		}
		
		$vcmp_pbtitleid= strtolower($vcmpprofilebox_title);
		$vcmp_ptid=preg_replace("/[^A-Za-z0-9]/", '', $vcmp_pbtitleid);
		
		$output .= '<div class="vcmpprofileboxwrapper '.esc_attr( $css_class ).' '.esc_attr( $el_class ).'" id="vcmpprofilebox-'.$vcmp_ptid.'">
					  <div class="vcmpprofileboxcard">
						<div class="vcmpprofileboxme" style="background: '.$vcmpprofilebox_bgcolor.' url('.$bigimage_array[0].') no-repeat scroll center center /cover"></div>
						<div class="vcmpprofileboxtext">
							<' . $font_container_data['values']['tag'] . ' ' . $style . '><a href="'.$vcmpprofilebox_url.'" ' . $style . '>'.$vcmpprofilebox_title.'</a></' . $font_container_data['values']['tag'] . '>
		...
		if ( $use_custom_for_content== 'true' ) {			
				$output .= '<style>#vcmpprofilebox-'.$vcmp_ptid.' .vcmpprofileboxtext * {font-family:'.$google_fonts_family[0].'; font-weight:'.$google_fonts_styles[1].'; font-style:'.$google_fonts_styles[2].' }</style>';
		}
